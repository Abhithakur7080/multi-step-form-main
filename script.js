...
const circleSteps = document.querySelectorAll('.step-top');
...
switcher.addEventListener('clcick', () => {
...
function switchPrice(checked) {
    ...
    if(checked){
        ...
        prices[1].innerHTML=`${yearlyPrice[5]}/yr`
        prices[2].innerHTML=`${yearlyPrice[2]}/yr`
        setTime(true);
    }
    else{
        prices[0].innerHTML=`${monthlyPrice[0]}/mo`
        prices[1].innerHTML=`${monthlyPrice[5]}/mo`
...
function setTotal() {
    ...
    const addonprice = document.querySelectorAll(".selected-addon .service-price");
    let val = 0;
    for(let i=0; i<addonprice.length; i++){
        const str = addonprice[i].innerHTML;
        const res = str.replace(/\D/g, "");
        val += Number(res);
    }
